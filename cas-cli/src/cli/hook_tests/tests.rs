...
fn test_configure_creates_settings() {
    ...
    if global_has_cas_hooks() {
        ...
        // Shell-form fixture: hook entries must carry "command" string, not "args" array.
        // Claude Code 2.1.139's /doctor rejects exec-form before agent load
        // (anthropics/claude-code#58441 — compat workaround until upstream ships a fix).
...
// =============================================================================
// Characterization tests for hook emission format (cas-c17b)
//
// cas-7ecd migrated emitters to exec-form "args" arrays.  cas-c17b reverts
// them back to shell-form "command" strings because Claude Code 2.1.139's
// /doctor validator rejects exec-form before the agent loads, blocking every
// factory worker spawn (anthropics/claude-code#58441).  exec-form remains
// recognised in duplicate_check.rs for backwards-compat with existing
// settings files generated by pre-cas-c17b CAS versions.
// =============================================================================
...
/// Confirm: hooks emitted by get_cas_hooks_config use the shell-form
/// `"command": "cas hook <Event>"` string.  cas-c17b reverts the exec-form
/// migration (cas-7ecd) to silence Claude Code 2.1.139's /doctor warning that
/// blocked every factory worker spawn (anthropics/claude-code#58441).
#[test]
fn hook_entries_emit_shell_form_command_string() {
    let config = get_cas_hooks_config(&HookConfig::default());

    for (event, expected_cmd) in &[
        ...
        assert_eq!(
            ...
            Some(*expected_cmd),
            "{event} hook must carry shell-form command string (cas-c17b)"
        );
    }
}

/// Confirm: hooks emitted by get_cas_hooks_config do NOT use exec-form
/// `"args": [...]` — only shell-form `"command": "..."` is emitted (cas-c17b).
/// Exec-form hooks in existing user settings files are still recognised by
/// has_cas_hook_entries — see test_exec_form_still_detected_by_has_cas_hook_entries.
#[test]
fn hook_entries_do_not_emit_exec_form_args() {
    let config = get_cas_hooks_config(&HookConfig::default());
    ...
    for event in &[
        ...
        assert_eq!(
            ...
            None,
            "{event} hook must not carry exec-form args array after cas-c17b shell-form revert"
        );
    }
}
...
/// in shell-form.  `first_hook_command` only returns the first entry; this test
/// uses `nth_hook_command(..., 1)` to reach the staleness-check entry explicitly.
/// cas-c17b reverted both SessionStart entries to shell-form; this guards the
/// check-staleness emitter specifically (config_gen.rs:172).
#[test]
fn session_start_check_staleness_emits_shell_form() {
    ...
    assert_eq!(
        ...
        "check-staleness entry under SessionStart must use shell-form command (cas-c17b)"
    );
    // Also confirm the second entry has no exec-form args leak.
    let staleness_args = config
        .pointer("/hooks/SessionStart/1/hooks/0/args");
    assert!(
        staleness_args.is_none(),
        "check-staleness entry must not carry exec-form args array"
    );
}
