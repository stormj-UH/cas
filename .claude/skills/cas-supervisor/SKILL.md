...
- **The close gate now validates commit reality (cas-490f).** When a worker provides `code_review_findings`, the gate rejects their close if `git rev-list base..HEAD --count == 0` — i.e. no commits on the branch despite a code-review claim. The close result also surfaces the actual `git diff --stat` so you can verify the delta matches the claim without inspecting the worktree manually. Trust the gate; if it accepts the close, commits exist. If a worker is stuck in a fabrication-detect loop, they either need to commit their work or retry without `code_review_findings` (for doc-only tasks that need no review envelope).
