...
1. **Get session context**: `mcp__cs__search action=context`
2. **List tasks touched**: `mcp__cs__task action=mine`
3. **Get recent memories**: `mcp__cs__memory action=recent limit=20`
4. **Check for open/in-progress tasks**: `mcp__cs__task action=list status=in_progress`
...
6. **Store summary**:
   ```
   mcp__cs__memory action=remember content="<summary>" title="Session Summary - [Date]" entry_type=context tags="session,summary"
   ```
